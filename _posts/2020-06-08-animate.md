...
然后第三个参数 除了贝塞尔曲线之外 还有 steps()函数

steps有两个参数,第一个是段数(正整数),第二个是类型(start,end),同时有两个关键字 step-strat,step-end
...
放在动画上,只有setInterval调用的才会有动画效果.也就是end与start动画执行次数相同,但是strat第一步没有动画效果,end则是没有最后一步的动画效果.我们可以给动画添加延迟看到strat的第一步,给动画添加填充forwards看到动画的最后一步.不多说了,上例子.
...
```
<div id="animate1" class="animation1" >12</div><div id="animate2" class="animation2" ></div>
