...
+ **GFC**  栅格格式上下文  [练习Grid布局的小游戏](http://cssgridgarden.com/)    display:grid
  ...
  + 容器属性
    + grid-template-columns和grid-template-rows定义每列的列宽和每列的行高,
      ...
      + auto-fill关键字, grid-tenplate-columns: repeat(auto-fill,300px),每列300px直到充满容器
      + fr,可视为单位,定义列比例grid-template-columns: 150px 1fr 2fr;二三列瓜分剩余宽度,第三列是第二列的两倍,
      + minmax() 指定大小范围 grid-template-columns: 1fr 1fr minmax(100px, 1fr);100px<=第三列<=1fr ; 有max-content和min-content关键字表示内容最大的单元格和内容最小的单元格. auto在max的时候表示max-content,在max表示min-width或min-height...需要注意当在minmax中使用fr时候要把他作为max值,不能当做min值
      + auto 浏览器决定,一般是剩余宽度 grid-template-columns: 100px auto 100px;
      + 可以为网格线具名 rid-template-columns: [c1] 100px [c2] 100px [c3] auto [c4];
