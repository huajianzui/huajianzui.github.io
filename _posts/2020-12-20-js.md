...
// 64位表示位  111 111 111 10(全为1时标记为±∞或NAN)    111 111 111 111 111 111 111 111 111 111 111 111 111 111 111 111 111 1  即 11位指数位和52位数值位,这52位数值位为小数点后的数值,默认前边有1 2**(2**11 - 1 -1 -1023)*数值位部分 = 2**1023* (2**53-1)*2**-52   = (2**53-1)*2**971 
//我们把(2**53-1)*2**971 复制到浏览器中  就是 1.7976931348623157e+308 
...
//如果另一个是对象,数值或布尔则调用先调用valueOf(),toString进行字符串拼接
[] + {}; //"[object Object]" 
...
当一个函数执行时,创建当前的执行环境,复制当前函数的[[scope]]中对象构建起当前执行环境的作用域链,把当前的活动对象(activation object)推入到作用域链的前端,执行环境被推入到执行栈中,在函数中使用变量时,会沿着作用域链向查找
函数执行之后执行环境从栈中弹出
...
Number.isFinite("12"); //false 相较于isFinite,不会进行类型转换  是否是无穷大
isFinite("12"); //true
...
Number.MIN_SAFE_INTEGER; // 保留3位向偶舍入 001001=> 001  001100=>010 000100=> 000
//向偶舍入 舍去部分的数值超过最低位数值一半则进位,低于则不进位等于时 最低位是1进位,0不进位
...
export {default as defaultModule} from "***"
